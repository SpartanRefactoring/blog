...
public interface Utils {
  /**
   * Appends an element to an array, by reallocating an array whose size is greater by one and placing the element at the last position.
   * @param < T >  JD
  ...
  /**
   * Removes the @Nullable annotation present on the type of a value. This function is mainly used to make <code><b>null</b></code> checkers happy. <p> The parameter is an instance of an arbitrary type, T. The hidden assumption is that a @Nullable annotation is present on T. Thus, the parameter may be either <code><b>null</b></code>, or an actual instance of T. <p> The function returns the same instance it received as a parameter, except that this instance is returned as an instance of the type T <i>without</i> the @Nullable annotation. Execution is aborted with an {@link AssertionError}  if the parameter is null. <p> As it turns out, this function is a (slow) logical no-op, but still applicable to arguments of type T, where T does not have the @Nullable annotation present on it. <p> For reasons related to the way non-nullability is managed in Java, the compiler will not warn you from doing applying this function to a {@link NonNull}  type. However, there is absolutely no point in removing a @Nullable annotation if the type that does not have it. Doing so a is plain clutter. Since the compiler cannot assist you, you will have to be on the guard.
   * @param < T >  an arbitrary type
   * @param $  an instance of the type parameter
   * @return  its parameter, after verifying that it is not <code><b>null</b></code>
  ...
  /**
   * Impose an ordering on type <code><b>boolean</b></code> by which <code><b>true</b></code> is greater than <code><b>false</b></code>.
   * @param b1  JD
   * @param b2  JD
   * @return  an integer that is negative, zero or positive depending on whether the first argument is less than, equal to, or greater than the second.
   * @see Comparable
   * @see Comparator
   */
   ...
   * @return  tree iff the the first parameter contains any of the substrings found in the second parameter
   */
  ...
  /**
   * Deletes a specified element from an array, by reallocating an array whose size is smaller by one and shifting the other elements down.
   * @param < T >  JD
   * @param ts  an arbitrary array
   ...
   * @return  <code><b>true</b></code> <i>iff</i> the index is valid index into the list. and it is the last one in it.
   */
   ...
   * @param < T >  JD
   * @param ts  JD
   * @return  the last item in a list or <code><b>null</b></code> if the parameter is <code><b>null</b></code> or empty
   */
   ...
   * @param o  JD
   * @param os  JD
   * @return  <code><b>true</b></code> <i>iff</i> the  {@link Object}  parameter is the same as the last element of the  {@link List}  parameter
   */
  ...
  /**
   * Aborts in case a given value is <code><b>null</b></code>. <p> This function is the lesser used dual of  {@link #cantBeNull(Object)} .
   ...
   * @param o  JD
   * @param os  JD
   * @return  <code><b>true</b></code> <i>iff</i> the an  {@link Object}  parameter occurs as the penultimate element of the  {@link List}  parameter
   */
   ...
   * @return  the  {@link StringBuilder}  parameter with the <code><b>char</b></code> parameter prepended to it
   */
   ...
   * @return  the  {@link StringBuilder}  parameter with the  {@link String} parameter prepended to it
   */
   ...
   * @return  <code><b>true</b></code> <em>iff</em>the file name ends with any one of the supplied extensions.
   */
   ...
   * @return  <code><b>true</b></code> <em>iff</em> <code>s</code> ends with any one of the supplied suffixes.
   */
