...
\cc{.Date} defined from \cref{figure:data:reap}.
...
outdated with respect to its prerequisites: Property \cc{toSring}
...
\end{adjustbox}
\end{figure}
 % Boolean positive() -> n < 0 == d < 0; // Including `$+∞$' and `$-∞$'
 % Boolean infinite() -> d == 0; // True in case not a number
 % Real abs() -> r > 0 ? r : -r; // Only defined if not `\cc{infinite}'
 % Boolean zero() -> abs == 0; // Only defined if not `\cc{infinite}'
...
\Cref{figure:rational:prerequisites} depicts prerequisites' graph
...
In \cref{figure:rational:prerequisites} we also see that recipes~\cc{r} and
\cc{positive} depend directly on ingredient~\cc{n}, and that recipe \cc{zero}
depends on recipe \cc{abs}, which in turn, depends on~\cc{r}. Therefore,
\cc{abs} and \cc{zero} \emph{indirectly} depend on~\cc{n} and~\cc{r}.
...
The value of a property is \emph{missing} if any one of the following conditions holds.
  ...
  \begin{enumerate}
    \item \emph{Empty cells:} We say that a property is \emph{free} if this property
        ...
        \cref{figure:rational} both ingredients~\cc{n} and~\cc{d} are initially
        free.

        To further the gastronomical parable, a cell may depend on empty cells---likewise, a
        recipe, such as \cc{positive}, may depend on free properties.

        If a property is \emph{free}, then its value is said to be
        \emph{missing}†{we also may say that the property itself is
        ...
        In the figure we employ a visual convention of drawing a border around
        non-missing values. The figure shows that values of~\cc{d} and
        \cc{infinite} are non-missing.

        To elaborate, lazy caching semantics means the assignment to~\cc{d}
        invalidates the cached value of properties that depend on it directly,
        ...
        As a visual hint, invalidated cache values are drawn in a brigher 
        background. The value on property \cc{infinite} is not shaded, 
        since the read of property \cc{infinite} triggers a computation of the
        recipe, which returns \kk{True}. 
        ...
        without changing its value.
          ...
          As an example, consider the state of affairs in~\cc{.Rational},
          after we have further applied:

        \begin{enumerate}
          \item assignment of~\cc{1} to property~\cc{d}; followed by,
          \item a read of property~\cc{positive}; and,
          \item a read of property~\cc{r}.
        \end{enumerate}

        The resulting prerequisites' is portrayed in \Cref{figure:rational:2}.
          ...
          To understand why, notice that assignment \cc{n=1} invalidated all
          properties except for \cc{infinite}. In particular, property~\cc{r} is
          invalidated by the assignment~\cc{n=1}. In reading~\cc{r},
          the recipe is triggered, since bot of~\cc{n} and~\cc{d}, the two
          prerequisites are no longer free.
          This recipe however aborts due to division by zero.

          The abortion makes property~\cc{r} retain its previous missing value.
          A subsequent read of~\cc{r} will trigger the recipe, unless one or
          more of~\cc{n} or~\cc{d} was updated prior to the read.

          The result of~\cc{r} retaining its missing value, makes recipes
          \cc{abs} and \cc{zero} retain their cached
          missing value as well.

          The read of \cc{positive} triggers recomputation of of its recipe,
          and this recomputation returns \kk{True}.†{In the~\cc{.Rational}
