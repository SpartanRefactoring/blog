...
\cref{table:comparisons-original} shows the aggregating statistics for a comparison between 
...
\begin{table}
  ...
  \emph{naming} together) and the \emph{structural} only}
  ...
  \begin{adjustbox}{max width=\columnwidth}
    \begin{tabular}{l*4r}
      \toprule
      & \multicolumn3c{\textit{Improvement of Total\footnote{Structural and Naming} over Structural Spartanization}}\\
% 	& \multicolumn1c{\textit{Improvement}}\\
%       \cmidrule(r){2-3}%\cmidrule(r){4-5} % \cmidrule(r){6-6}
      & \textit{Jack}
      & \textit{BZip2 after Jack}
