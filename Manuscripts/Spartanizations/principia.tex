...
For example, function \cc{equals(Object ¢)} in the figure is described by its
...
of \kk{this}. Futher, the parameter must also be of the same as class as 
\kk{this}, and equal to \kk{this} when downcasted to its class.
\end{quote}
...
\begin{itemize}
  \item \emph{Cleanup}, adding \kk{final}s and \cc{@Override} annotations, 
    ...
    \item \emph{Nominal spartanization}, including renaming of the unused
      parameter \cc{args} to \cc{main($\cdot$)}, to the empty spartan name
      \cc{\_\_}.

      Nominal spartanization also renamed certain variables to~\cc{¢}, which is
      the spartan name for what might be denoted by keyword \kk{that} in a
      redesigned language, and is the most imminent argument or manipulated
      entity that is not \cc{this}. 
\end{itemize}
...
Overall, the result of automatic application of spartanization
  ...
  is due to cleanup operation that optimizes the use of \kk{import}; we shall


Evidently
Even on the pedestrian code such as that of \cref{figure:eclipse}
spartanization has an impact.
such as
