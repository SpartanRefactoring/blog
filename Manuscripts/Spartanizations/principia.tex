...
\begin{enumerate}
  ...
  \item \textit{horizontal complexity,} including, the number of parameters to a
        ...
        control commands, \kk{if}, \kk{while}, \kk{switch}, \emph{and} maximal
...
\end{java}
into to \cc{\kk{return} !f();}, including a wring to remove redundant
...
\begin{figure}
  \caption{A spartanized version of the \Java class in \cref{figure:cell9}
    automatically generated methods (16 lines, 70 words and 420 characters).}
    \label{figure:cell1}
    \begin{adjustbox}{max width=\columnwidth}
\begin{code}[minipage, width=1.25\columnwidth]{JAVA}
public class C1<T> {
...
the ternary operator and the short-circuit operators, ‟\cc{\|\|}” and
‟\cc{＆＆}”. Spartanization also abbreviated \cc{obj} to \cc{o}.  The code is
evidently shorter, not as nested.  Horizontal complexity, i.e., line length has
improved as well, except for an increase due to the extraction of a
\kk{private} version of \cc{equals($\cdot$)}; recall though that this stage is
carried out manually, at user's discretion.
...
the this pedestrian code. Employed were cleanup operations (adding \kk{final}s
and \cc{@Override} annotations, and the convoluted reference to \cc{names}),
